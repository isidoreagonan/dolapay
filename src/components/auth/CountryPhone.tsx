...
export function CountrySelect({
  ...
  return (
    <div className="block">
      <span className="mb-1.5 block text-xs font-semibold text-slate-900/80">{label}</span>
  ...
  return (
    <label className="block">
      ...
      <div className="flex items-stretch gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 focus-within:border-primary">
        <div className="flex shrink-0 items-center gap-1.5 border-r border-slate-200 pr-3 text-sm font-semibold text-slate-900/80">
        ...
        <input
          ...
          required
          className="min-w-0 flex-1 bg-transparent text-sm text-slate-900 placeholder:text-slate-500/60 focus:outline-none disabled:opacity-60"
        />
      </div>
    </label>
  );
