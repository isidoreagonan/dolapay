...
export const Route = createFileRoute("/_authenticated/dashboard/payment-links_/$id/edit")({
  component: EditPaymentLinkPage,
});
...
function EditPaymentLinkPage() {
  ...
  useEffect(() => {
    if (link) {
      ...
      setCurrency((link.currency as Currency) ?? "XOF");
      setImageUrl(link.image_url ?? "");
      setInvoiceNumber(link.invoice_number ?? "");
      setFeesPaidBy((link.fees_paid_by as FeesPaidBy) ?? "merchant");
      ...
      if (link.theme_config) {
        ...
        if (tc.buttonTextColor) setButtonTextColor(tc.buttonTextColor);
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["payment-link", id] });
  ...
  if (error || !link) {
    return (
      <div className="mx-auto max-w-4xl pt-12 text-center">
        <p className="text-rose-500">Erreur lors du chargement du lien.</p>
        <Button className="mt-4" onClick={() => navigate({ to: "/dashboard/payment-links" })}>Retour</Button>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black tracking-tight">Modifier le lien</h1>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex items-center justify-end gap-3 rounded-2xl border border-border/60 bg-background/60 p-4 backdrop-blur shadow-sm">
          ...
          <Button type="submit" size="lg" disabled={save.isPending} className="gap-2 px-8">
            ...
            {save.isPending ? "Mise à jour…" : "Sauvegarder les modifications"}
          </Button>
        </div>
      </form>
    </div>
