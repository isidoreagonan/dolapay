-- ==========================================================================
-- SCRIPT DÉFINITIF : RÉPARATION GOTRUE AUTH + COLONNES MANQUANTES + RESTAURATION
-- ==========================================================================

-- 1. AJOUT DES COLONNES MANQUANTES POUR ÉVITER L'ERREUR POSTGREST (PGRST204)
ALTER TABLE public.profiles
  ...
  ADD COLUMN IF NOT EXISTS verification_mode TEXT DEFAULT 'manual',
...
ALTER TABLE public.payment_links
  ADD COLUMN IF NOT EXISTS merchant_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS image_url TEXT,
  ADD COLUMN IF NOT EXISTS invoice_number TEXT,
  ADD COLUMN IF NOT EXISTS thank_you_message TEXT,
  ADD COLUMN IF NOT EXISTS fees_paid_by TEXT DEFAULT 'customer';

-- 2. NETTOYAGE DES ID TEMPORAIRES SUR LE NOUVEAU SUPABASE
...
-- 3. CRÉATION DES COMPTES ORIGINAUX DANS AUTH.USERS AVEC LES VALEURS DE BASE GOTRUE
INSERT INTO auth.users (
  instance_id, id, aud, role, email, encrypted_password, email_confirmed_at, confirmation_token, recovery_token, email_change_token_new, email_change, is_super_admin, is_sso_user, raw_app_meta_data, raw_user_meta_data, created_at, updated_at
) VALUES (
...
-- RÉPARATION DU MOTEUR GOTRUE POUR TOUS LES COMPTES EXISTANTS (ÉLIMINE L'ERREUR 500 / { })
UPDATE auth.users
SET
...
-- 4. RESTAURATION DES PROFILS RÉELS AVEC GARANTIE ENTERPRISE SUR ISIDORE
INSERT INTO public.profiles (id, email, full_name, account_type, kyc_status, volume_limit_xof, onboarding_completed, created_at, updated_at) VALUES ('1e1934d4-4216-4140-98cc-0510d80e8860', 'isidoreagonan@gmail.com', 'AGONAN ISIDORE ABRAHAM', 'enterprise', 'approved', 999999999999, true, '2026-06-25 05:09:37.196719+00', '2026-06-25 05:10:20.150922+00') ON CONFLICT (id) DO UPDATE SET full_name = EXCLUDED.full_name, account_type = EXCLUDED.account_type, kyc_status = EXCLUDED.kyc_status, onboarding_completed = true;
INSERT INTO public.profiles (id, email, full_name, account_type, kyc_status, volume_limit_xof, onboarding_completed, created_at, updated_at) VALUES ('85a8913c-7ad2-40fc-913c-41678796ab16', 'astucegpt@gmail.com', 'AGONAN ISIDORE', 'standard', 'pending', 1000000, false, '2026-06-25 04:11:44.973341+00', '2026-06-25 14:54:07.53157+00') ON CONFLICT (id) DO UPDATE SET full_name = EXCLUDED.full_name, account_type = EXCLUDED.account_type, kyc_status = EXCLUDED.kyc_status;
INSERT INTO public.profiles (id, email, full_name, account_type, kyc_status, volume_limit_xof, onboarding_completed, created_at, updated_at) VALUES ('de03c351-094d-4b7d-a692-e10be7373f27', 'dolapoecom1@gmail.com', 'Dolapo ECOM', 'enterprise', 'approved', 1000000, true, '2026-06-25 04:05:29.712303+00', '2026-06-26 10:12:40.33056+00') ON CONFLICT (id) DO UPDATE SET full_name = EXCLUDED.full_name, account_type = EXCLUDED.account_type, kyc_status = EXCLUDED.kyc_status;

-- 5. RESTAURATION DE L'ENTREPRISE RÉELLE
...
-- 6. RESTAURATION DES RÔLES (ADMIN & MARCHAND)
...
-- 7. RESTAURATION DES VRAIS LIENS DE PAIEMENT
