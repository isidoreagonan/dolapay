-- ==========================================================================
-- SCRIPT DE SEED / DONNÉES DE DÉMO LOVABLE POUR DOLAPAY
-- Ce script injecte des comptes vérifiés (Enterprise & Standard),
-- des entreprises, des liens de paiement et des transactions réalistes.
-- À exécuter dans le SQL Editor de Supabase (sdieqwzrggypjpkjiogg).
-- ==========================================================================

DO $$
BEGIN
  -- 1) Insertion des utilisateurs dans auth.users (si la table auth.users est accessible)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Insertion dans auth.users ignorée ou non autorisée : %', SQLERRM;
  END;

  -- 2) Désactiver temporairement la contrainte FK sur profiles si auth.users n'a pas pu être inséré
  BEGIN
    ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_id_fkey;
  EXCEPTION WHEN OTHERS THEN NULL;
  END;

  -- 3) Insertion dans public.profiles
  ...
  -- 4) Insertion dans public.businesses
  ...
  -- 5) Insertion des Liens de Paiement (payment_links)
  ...
  -- 6) Insertion de Transactions réalistes (transactions)
